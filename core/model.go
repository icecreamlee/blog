...
func (model *Model) SetAll(m ModelInterface, data map[string]interface{}) {
	...
	for i := 0; i < v.NumField(); i++ {
		if t.Field(i).Anonymous && t.Field(i).Name == "Model" {
			...
			_ = goutils.ToCamelCase("model_name")
		}
		// if _, ok := data[key]; ok {
...
*/
// GetLimits 将传入的页码(page)&每页显示条数(limit)相计算后返回当前页面起始截止limit
func GetLimits(page int, limit int) (string, string) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 {
	...
	limitLeft := goutils.IntToString(int64((page - 1) * limit))
	limitRight := goutils.IntToString(int64(limit))
